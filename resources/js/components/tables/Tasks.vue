    ...
    <b-row align-v="center" class="mt-3">
      <b-col cols="3">
        <VueCtkDateTimePicker
          id="tasks-start" label="From next due"
          input-size="sm"
      ...
      <b-col cols="3">
        <VueCtkDateTimePicker
          id="tasks-end" label="To next due"
          input-size="sm"
      ...
      <b-col cols="6" class="mt-2">
        <b-form-group label="Filter" label-cols-sm="1" label-align-sm="right" label-size="sm" label-for="filterInput">
          <b-input-group size="sm">
            <b-form-input v-model="table.filter" debounce="500" type="search" id="filterInput" placeholder="Search task name"></b-form-input>
    ...
    <b-row>
      <b-col cols="12">
        ...
        >
          ...
          <template v-slot:cell(due)="row"> 0
            <div class="data">
              {{ row.item.data.due }}
            </div>
          </template>
          ...
          <template v-slot:row-details="row">
            <b-card border-variant="light" class="font-size-small w-100">
              <b-row v-if="row.item.data" class="mb-2">
                <b-col cols="12">{{ 'description' in row.item.data ? row.item.data.description : ''}}</b-col>
              </b-row>
            </b-card>
          </template>
        </b-table>
...
export default {
  ...
  data() {
    return {
      ...
      table: {
        fields: [
          { key: 'data.name', sortable: true, style: 'width:30%', label: 'To do' },
          { key: 'data.due', sortable: true, style: '', label: 'Due' },
          { key: 'isActive', sortable: true, style: '' },
          { key: 'name', sortable: true, style: '', label: 'Owner'},
  ...
  methods: {
      ...
      dateTimePicked($event, dateTime, action) {
        ...
        try {
          typeof this.$refs.taskTable.refresh()
        } catch(err){
          console.log(err)
          return
        }
      ...
      // async table loader for Table provider, called on init and updates
